...
return new class extends Migration
{
    ...
    {
        Schema::table('customer_favorite_services', function (Blueprint $table) {
            // Add salon_id to scope favorites per customer-salon pair
            if (!Schema::hasColumn('customer_favorite_services', 'salon_id')) {
        ...
        try {
            DB::transaction(function () {
                Schema::table('customer_favorite_services', function (Blueprint $table) {
                    $table->dropUnique(['customer_id', 'service_id']);
                });
            });
        } catch (\Exception $e) {
        ...
        Schema::table('customer_favorite_services', function (Blueprint $table) {
            $table->unique(['customer_id', 'salon_id', 'service_id']);
        });
    }
    ...
    {
        Schema::table('customer_favorite_services', function (Blueprint $table) {
            $table->dropUnique(['customer_id', 'salon_id', 'service_id']);
        });
