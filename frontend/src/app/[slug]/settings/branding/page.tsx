...
export default function SalonBrandingSettingsPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    if (!user) {
      router.push(`/login?returnTo=/${slug}/settings/branding`);
      return;
    }

    if (!salonId) {
      console.warn('[SalonBrandingSettingsPage] User has no salon assignment');
      return;
    }

    // Verify the salon slug matches the user's active salon
    if (activeSalon && activeSalon.slug !== slug) {
      console.error('[SalonBrandingSettingsPage] Access denied: user salon does not match URL salon');
