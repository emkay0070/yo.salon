...
function UsageCard({ metric }: { metric: UsageMetric }) {
  ...
  const isNotIncluded = metric.limit === 0;
  const isWarning = !isUnlimited && !isNotIncluded && metric.is_near_limit && !metric.is_over_limit;
  const isCritical = (!isUnlimited && metric.is_over_limit) || (isNotIncluded && metric.current_value > 0);
    ...
    <div className={`bg-card border rounded-xl p-4 ${
      ...
      <div className="flex items-end justify-between mb-2">
        <div>
          <span className={`text-2xl font-bold ${isCritical ? 'text-red-400' : isWarning ? 'text-amber-400' : 'text-text-primary'}`}>
            {isNotIncluded && metric.current_value === 0 ? '—' : metric.current_value}
          </span>
          <span className="text-text-secondary text-sm ml-1">
            {isUnlimited ? '/ ∞' : isNotIncluded ? '' : `/ ${metric.limit}`}
          </span>
        </div>
        <span className={`text-xs font-medium ${isCritical ? 'text-red-400' : isWarning ? 'text-amber-400' : 'text-text-secondary'}`}>
          {isUnlimited ? 'Unlimited' : isNotIncluded ? 'Not included' : `${metric.remaining} remaining`}
        </span>
      </div>
      ...
      <div className="h-2 bg-surface rounded-full overflow-hidden">
        <div
          ...
          style={{ width: isUnlimited ? '100%' : isNotIncluded ? (isCritical ? '100%' : '0%') : `${Math.min(metric.percentage, 100)}%` }}
        />
      </div>
    </div>
  );
...
export default function SalonMembershipPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    if (!user) {
      router.push(`/login?returnTo=/${slug}/settings/membership`);
      return;
    }

    if (!salonId) {
      console.warn('[SalonMembershipPage] User has no salon assignment');
      return;
    }

    // Verify the salon slug matches the user's active salon
    if (activeSalon && activeSalon.slug !== slug) {
      console.error('[SalonMembershipPage] Access denied: user salon does not match URL salon');
        ...
        {showUpgradeModal && (
          ...
          >
            ...
            <motion.div
              ...
              className="relative inset-4 md:inset-auto md:max-w-6xl md:w-full bg-surface border border-border-medium rounded-2xl shadow-2xl overflow-hidden"
            >
