...
// ─── Component ────────────────────────────────────────────────────────────────
export default function SalonPaymentsPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    if (!user) {
      router.push(`/login?returnTo=/${slug}/payments`);
      return;
    }

    if (!salonId) {
      console.warn('[SalonPaymentsPage] User has no salon assignment');
      return;
    }

    // Verify the salon slug matches the user's active salon
    if (activeSalon && activeSalon.slug !== slug) {
      console.error('[SalonPaymentsPage] Access denied: user salon does not match URL salon');
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-white/70 text-sm mb-2">Merchant ID</label>
                <input
                  type="text"
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button
                ...
                className="flex-1 px-4 py-2 bg-blue-500/20 hover:bg-blue-500/30 border border-blue-500/30 rounded-lg text-blue-400 transition-colors disabled:opacity-50"
              >
                {addVerifying ? 'Adding...' : 'Add Method'}
              </button>
            </div>
          </motion.div>
        </div>
      ...
      {isDepositModalOpen && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <motion.div
            ...
            className="bg-[#0c0c0c] border border-white/10 rounded-2xl p-6 w-full max-w-lg"
          >
            <h2 className="text-xl font-semibold text-white mb-6">Deposit Policy Settings</h2>

            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <label className="text-white/70 text-sm">Enable Booking Deposits</label>
                <input
                  type="checkbox"
                  checked={depositFormData.booking_deposit_enabled}
                  onChange={(e) => setDepositFormData({ ...depositFormData, booking_deposit_enabled: e.target.checked })}
              ...
              <div>
                <label className="block text-white/70 text-sm mb-2">Deposit Value</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-white/70 text-sm mb-2">Required For</label>
                <select
                ...
                >
                  ...
                  <option value="high_value">High Value Services Only</option>
                </select>
              </div>
            </div>
