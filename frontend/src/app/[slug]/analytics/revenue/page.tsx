...
export default function SalonRevenueAnalytics({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    if (!user) {
      router.push(`/login?returnTo=/${slug}/analytics/revenue`);
      return;
    }

    if (!salonId) {
      console.warn('[SalonRevenueAnalytics] User has no salon assignment');
      return;
    }

    // Verify the salon slug matches the user's active salon
    if (activeSalon && activeSalon.slug !== slug) {
      console.error('[SalonRevenueAnalytics] Access denied: user salon does not match URL salon');
