...
export default function SalonAnalyticsPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    if (!user) {
      router.push(`/login?returnTo=/${slug}/analytics`);
      return;
    }

    if (!salonId) {
      console.warn('[SalonAnalyticsPage] User has no salon assignment');
      return;
    }

    // Verify the salon slug matches the user's active salon
    if (activeSalon && activeSalon.slug !== slug) {
      console.error('[SalonAnalyticsPage] Access denied: user salon does not match URL salon');
