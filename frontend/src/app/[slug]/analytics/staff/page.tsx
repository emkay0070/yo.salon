...
export default function SalonStaffAnalytics({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    if (!user) {
      router.push(`/login?returnTo=/${slug}/analytics/staff`);
      return;
    }

    if (!salonId) {
      console.warn('[SalonStaffAnalytics] User has no salon assignment');
      return;
    }

    // Verify the salon slug matches the user's active salon
    if (activeSalon && activeSalon.slug !== slug) {
      console.error('[SalonStaffAnalytics] Access denied: user salon does not match URL salon');
